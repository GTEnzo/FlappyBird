...
current_background = 'background1.jpg'  # Только имя файла
...
def settings_window():
    global flap_sound, current_background
    ...
    current_bg_index = backgrounds.index(current_background)
    ...
    while running:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if back_button_rect.collidepoint(event.pos):
                    running = False
                # Обработка нажатия на кнопки выбора фона
                if next_bg_button_rect.collidepoint(event.pos):
                    current_bg_index = (current_bg_index + 1) % len(backgrounds)
                    current_background = backgrounds[current_bg_index]
                if prev_bg_button_rect.collidepoint(event.pos):
                    current_bg_index = (current_bg_index - 1) % len(backgrounds)
                    current_background = backgrounds[current_bg_index]
        ...
        bg_prev = pygame.transform.scale(load_image(backgrounds[current_bg_index]), (200, 150))
        screen.blit(bg_prev, (125, 300))

        # Кнопки для выбора фона
        prev_bg_button = pygame.Surface((50, 50))
        prev_bg_text = font.render('<', True, BLACK)
        prev_bg_rect = prev_bg_text.get_rect(center=(prev_bg_button.get_width() / 2, prev_bg_button.get_height() / 2))
        prev_bg_button_rect = pygame.Rect(50, 350, 50, 50)

        next_bg_button = pygame.Surface((50, 50))
        next_bg_text = font.render('>', True, BLACK)
        next_bg_rect = next_bg_text.get_rect(center=(next_bg_button.get_width() / 2, next_bg_button.get_height() / 2))
        next_bg_button_rect = pygame.Rect(350, 350, 50, 50)

        if prev_bg_button_rect.collidepoint(pygame.mouse.get_pos()):
            pygame.draw.rect(prev_bg_button, (141, 199, 63), (1, 1, 48, 48))
        else:
            pygame.draw.rect(prev_bg_button, (255, 251, 214), (1, 1, 48, 48))
        prev_bg_button.blit(prev_bg_text, prev_bg_rect)
        screen.blit(prev_bg_button, (prev_bg_button_rect.x, prev_bg_button_rect.y))

        if next_bg_button_rect.collidepoint(pygame.mouse.get_pos()):
            pygame.draw.rect(next_bg_button, (141, 199, 63), (1, 1, 48, 48))
        else:
            pygame.draw.rect(next_bg_button, (255, 251, 214), (1, 1, 48, 48))
        next_bg_button.blit(next_bg_text, next_bg_rect)
        screen.blit(next_bg_button, (next_bg_button_rect.x, next_bg_button_rect.y))
        ...
        back_rect = back_text.get_rect(center=(back_button.get_width() / 2, back_button.get_height() / 2))
...
def game_screen():
    ...
    while True:
        ...
        if pygame.sprite.collide_mask(bird, ground) or pygame.sprite.spritecollide(bird, pipes, False) or bird.rect.y < -200:
